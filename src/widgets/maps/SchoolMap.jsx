...
export function SchoolMap({ schools }) {
  ...
  const handleRangeSelect = async (building, rangeKey, metric) => {
    ...
    const data = await fetchHistoricData(building.serviceUri, metric, `${from}`, `${to}`);
  ...
  return (
    <div className="w-full h-full flex flex-col gap-4">
      ...
      <div className="flex w-full gap-4">
        ...
        {/* Sidebar */}
        <div className="w-64 bg-white rounded-xl shadow-md border p-4 flex-shrink-0"
             style={{ height: "450px", overflowY: "auto" }}>
          <h4 className="text-sm font-bold text-blue-gray-700 mb-2">Select Schools</h4>
        ...
        <div className="flex-1 rounded-xl overflow-hidden border shadow-md"
             style={{ height: "450px" }}>
          <MapContainer center={[36.44, 28.22]} zoom={11} scrollWheelZoom className="w-full h-full">
            ...
            {filtered.map((s, i) => (
              <Marker key={i} position={[s.lat, s.lng]}
                ...
                <Popup>
                  ...
                  <br />
                  <small className="text-gray-600">Last update: {s.dateObserved ?? "N/A"}</small>
                  <hr />
                  ...
                  {s.LVOC !== undefined && (
                    <>
                      {s.co2 !== null && (
                        <MetricRow label="CO₂" value={s.co2} unit="ppm"
                                   metric="co2" building={s} onRangeSelect={handleRangeSelect} />
                      )}
                    ...
                    <>
                      ...
                      {s.wind_speed !== null && (
                        <MetricRow label="Wind Speed" value={s.wind_speed} unit="km/h"
                                   metric="wind_speed" building={s} onRangeSelect={handleRangeSelect} />
                      )}

                      {s.wind_direction !== null && (
                        <MetricRow label="Wind Direction" value={s.wind_direction} unit="°"
                                   metric="wind_direction" building={s} onRangeSelect={handleRangeSelect} />
                      )}

                      {s.precipitation !== null && (
                        <MetricRow label="Precipitation" value={s.precipitation} unit="mm"
                                   metric="precipitation" building={s} onRangeSelect={handleRangeSelect} />
                      )}

                      {s.pressure !== null && (
                        <MetricRow label="Pressure" value={s.pressure} unit="hPa"
                                   metric="pressure" building={s} onRangeSelect={handleRangeSelect} />
                      )}
                    </>
                  )}
